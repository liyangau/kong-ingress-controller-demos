...
printf "curl -i http://localhost:8000/basic \n\n" 

curl -sv http://localhost:8000/basic | jq

printf "\nNext we are going to calculate and use our token. \n\n"
printf "curl -sv \"http://localhost:8000/basic\" -H \"Authorization:Basic $token\" \n\n"
pause
# Curl the service once with a consumers credentials to fill the rate limit
# Curl the service
token=$( echo "testuser:testpass" | base64 )
curl -sv "http://localhost:8000/basic" -H "Authorization:Basic $token" | jq
